# Inventory Button Registry — Doc 2 v1
...
**Source-of-truth files cited in this registry:**
- `service/app/services/inventory_state_engine.py` (states + transitions, lines 74–96; `transition()` lines 207–310; `get_state` 134, `get_history` 146, `list_by_state` 159, `count_by_state` 177)
- `service/app/api/routes_inventory.py` (read-only router prefix `/api/v1/inventory`, lines 19–23; only existing endpoint `GET /stage2/aggregate` line 42)
...
- `service/app/static/dashboard.html:1260-1530` — `InventoryPage`, including the 5 disabled buttons at 1329–1343 and the Stage 2 tile grid at 1442–1484
...
- No `inventory_allocations` table has been built. Buttons #5–#9 below depend on this table existing or on a chosen interim shim.

**Lifecycle states (current — `STATES` frozenset, `inventory_state_engine.py:81-85`):**
`PURCHASE_TRANSIT`, `WAREHOUSE_STOCK`, `DIRECT_DISPATCH_READY`, `CLIENT_DISPATCHED`, `SALES_TRANSIT`, `CLOSED`. Sample / consignment / return / quarantine sub-states are NOT in `STATES`; the 5 disabled buttons at `dashboard.html:1329-1343` target lifecycle outcomes that have no backing state today.

**Move stock is location-metadata only.** It updates physical location in `warehouse_db` movement events without calling `transition()`. No state change.
...
**Blocked-on-operator decisions (must resolve before Phase 6):**
- Allocation-only vs new-state model for samples/consignment/returns (#5, #6, #7, #8).
- Whether Doc 1 v2 §5 allocation enum needs a `RETURN` type added, or whether returns map to `QUARANTINE` (#8).
- Customs-doc workflow + Proforma-eligibility blocking for Return-to-producer (#9).
- Confirm Move stock is metadata-only with no `transition()` call (#2 security review).
