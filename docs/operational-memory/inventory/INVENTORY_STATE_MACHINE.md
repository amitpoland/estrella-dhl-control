...
- **Core principle — single writer:** every state change goes through `inventory_state_engine.transition()` (`service/app/services/inventory_state_engine.py:291`, line shifted from `:207` after Phase B.1 / B.2 additions). Every other code site reads from `inventory_state` or computes derived views. There are no parallel writers and no future code path may introduce one.
...
- `inventory_state_engine.transition()` at `inventory_state_engine.py:291` — adds two new entries to `LEGAL_TRANSITIONS` (`inventory_state_engine.py:132`) and the corresponding allowed-reason set; same shape as existing entries. No structural change to the function body.
- `STATES` frozenset at `inventory_state_engine.py:94` — append `RESERVED_FOR_PROFORMA` and `DISPATCH_PENDING`. **Note (Issue #27 refresh, 2026-05-13):** the on-main `STATES` frozenset already contains 9 members — the original 6 Phase A states (`PURCHASE_TRANSIT`, `WAREHOUSE_STOCK`, `DIRECT_DISPATCH_READY`, `CLIENT_DISPATCHED`, `SALES_TRANSIT`, `CLOSED`) **plus** the live Phase B.1 / B.2 additions (`SAMPLE_OUT` at `inventory_state_engine.py:83`, PR #17/#18; `RETURNED_FROM_CLIENT` at `:91` and `RETURNED_TO_PRODUCER` at `:92`, PR #22). The Doc 1 v2 design adds `RESERVED_FOR_PROFORMA` and `DISPATCH_PENDING` on top of those 9.
- `PROFORMA_ELIGIBLE_STATES` at `inventory_state_engine.py:197` — no change. `RESERVED_FOR_PROFORMA` is **not** eligible for a *new* proforma (that's the point of the reservation).
...
- **Endpoint contract**
  ...
  - Response 200: `{ "batch_id": str, "counts": {state: int}, "pieces": [{scan_code, product_code, design_no, state, updated_at}] }` — `counts` derived from `ise.count_by_state(batch_id)` (`inventory_state_engine.py:261`); `pieces` from a new helper joining `inventory_state` rows for the batch. **On-main note:** the endpoint `GET /api/v1/inventory/state/{batch_id}` is already live (`routes_inventory.py:74`).
...
- **Endpoint contract**
  ...
  - Response 200: `{ scan_code, product_code, design_no, batch_id, state, updated_at, updated_by, note, history: [{from_state, to_state, trigger, occurred_at, operator, note}] }` — composes `ise.get_state()` (`inventory_state_engine.py:218`) + `ise.get_history()` (`:230`). **On-main note:** the endpoint `GET /api/v1/inventory/pieces/{piece_id}` is already live (`routes_inventory.py:57`).
...
- All FK/consistency invariants that cannot be expressed as a SQLite FK (because `reservation_queue` lives in `reservations.db` and `inventory_state` lives in `warehouse.db`) are enforced inside the bridge code under the existing `_lock` (`inventory_state_engine.py:201`, formerly `:117`) plus the worker's transactional update boundary.
- `inventory_state_engine.transition()` already serialises all writes via `_lock` + a per-call SQLite transaction (`inventory_state_engine.py:291`, formerly `:235`). The bridge inherits this serialisation by calling `transition()` rather than touching the table directly.
