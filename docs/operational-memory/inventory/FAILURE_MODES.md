...
### Specific concerns
- Sample-out is now LIVE on main (PR #17 / #18). `SAMPLE_OUT` is a declared state (`inventory_state_engine.py:83`) and a member of the `STATES` frozenset (`inventory_state_engine.py:94`). `LEGAL_TRANSITIONS` (`inventory_state_engine.py:132`) carries the WAREHOUSE_STOCK ↔ SAMPLE_OUT pair. The failure modes in the row table above describe operational concerns, not pre-implementation design risks.
...
### Specific concerns
- `SAMPLE_OUT` is now a live state on main (`inventory_state_engine.py:83`, in `STATES` frozenset at `inventory_state_engine.py:94`). Sample-return is wired via the live piece-drawer form (PR #17/#18 + `routes_inventory_sample.py`). The failure mode in row 6 above is operational rather than design-blocked: a `sample-return` for a `scan_code` that is not currently in `SAMPLE_OUT` is rejected by the engine's evidence gate in `transition()` (`inventory_state_engine.py:291`).
