...
## SELF-ANALYSIS — 2026-08-22 CMR reproduction (ARCHAEOLOGIST + ADVERSARY co-signed)

**A campaign was scoped from a parenthetical.** Python appends *"(most likely due to a
circular import)"* to every `ImportError` on a partially initialized module. It is a
guess the interpreter makes with no knowledge of the import graph. That guess became a
root cause, an architectural goal, a prescribed dependency direction and a named
module to edit — and the dependency it told us to remove does not exist. **The
interpreter's hypothesis was read as the interpreter's finding.**

**Reproduce-before-repair is what stopped it**, and it stopped it three times: the
modules import cleanly in isolation and in every order; all 79 existing CMR tests pass;
and the live production endpoint answers 200 on five drafts. Any one of those would
have been enough to notice; going straight to the prescribed edit would have removed a
lazy import, seen the tests pass, and shipped a change whose justification was false
— with the real intermittent failure untouched.

**The disconfirming evidence came from the log, not the code.** One traceback, first
request after a restart, nothing since — and the same error text for
`accounting_analytics`, a module pair with no relationship to CMR. Two occurrences of
one mechanism beats one occurrence of two cycles.

**ARCHAEOLOGIST**: the lazy imports are original to both modules and byte-identical
between `7a241604` and `fd4c79a7`; no recent change caused this, which is exactly what
the CT-MASTER closure already recorded before ruling it a pre-existing latent defect.
**ADVERSARY**: my own leading explanation is NOT proven — the two-thread reproduction I
built to confirm it failed to reproduce it, and I am recording that failure rather than
quietly switching to a hypothesis I like better. What is proven is the negative: there
is no cycle here. Anything built on the positive claim needs the cross-lock case
demonstrated first, or it is the same mistake one level down.

## SELF-ANALYSIS — 2026-08-22 CMR outcome B (ARCHAEOLOGIST + ADVERSARY co-signed)

**Widening the census changed the campaign twice.** First from "a CMR cycle" to "no
cycle". Then from "two incidents" to **17 occurrences across five modules over four
months**, which reclassifies it from a bug in a module to a property of how this
application imports. The second widening cost one grep and was worth more than the
whole architectural theory that preceded it. The rule that earned it is already
doctrine: *findings must be cross-checked, not just filed* — applied here to the log,
not to the ledger.

**The strongest evidence was positional, not textual.** Every occurrence sits at
line ~57–61 of a ~60-line file: the first ASGI traceback of the process, once per
lifetime. No message content establishes that; only counting where they fall does. A
signature that regular is a structural fact even when its mechanism is unknown.

**Two reproductions were built and both failed, and that is recorded as the result.**
Same-module concurrent import: CPython serialises it correctly. Cross-lock pair with
opposite ordering: still succeeded. Under EVIDENCE SCALES WITH THE MUTATION, hoisting
an import is a change to application startup ordering; it would have made the
traceback disappear while leaving the reason unknown, and "the error stopped" is not
evidence that the cause was addressed.

**ARCHAEOLOGIST**: the lazy-import style predates every module implicated and is
uniform across the tree — nothing recent introduced it, and the CT-MASTER closure had
already reached the same conclusion for the CMR instance before ruling it pre-existing.
**ADVERSARY**: outcome B is a decision to leave a real, recurring production error in
place. It is defensible only because the error is once-per-process, self-clearing, and
observed healthy on five live drafts — and only for as long as that stays true. The
...
|---|---|---|---|---|---|
| F-01 | BLOCKER | Identity function not stable under a missing optional field | `packing_db.py:44`; two forms of one line → two keys | S0 ✅ key shipped | packing_db.py:44; 2 forms of one line |
| F-02 | BLOCKER | Root cause: `upsert_packing_lines` dedup key **branches** on `pack_sr` and is batch-scoped, so the two forms can never match and cross-batch duplicates are structurally invisible | `packing_db.py:765-789` | S0 (wiring remaining) | dedup key branches on pack_sr; batch-scoped |
| F-12 | **HIGH** | **38 duplicate line-keys, 39 surplus rows, 7 batches** — far beyond the census's 3. Includes one packing list ingested into two different shipment batches | shipped classifier over 1326 live rows | S0 + ST-02 | 38 duplicate keys; 39 surplus rows; 7 batches (census said 3) |
| F-13 | **HIGH** | Duplication propagated into `documents.db`: two supplier-invoice documents per duplicated packing document. Each states the true quantity, so the per-document money is right; the aggregate double-counts | `invoice_lines` for `EJL/26-27/177-5`: two documents × 20.0 | ST-02 | 2 supplier-invoice documents per duplicated line |
| F-14 | MEDIUM | 12 product_codes report an inflated import quantity (`EJL/26-27/177-5`: 40 reported vs 20 true) | quantity impact probe | S0 | 12 product_codes; EJL/26-27/177-5 40 vs 20 |
| F-03 | HIGH | 245 orphan `packing_lines` (15%) reference a non-existent document; no FK declared | batch `…_999deef1` | S0 | 245 orphan rows = 15%; no FK declared |
| F-15 | MEDIUM | Parser non-determinism: identical bytes yield 21 rows as `final`, 24 as `advance` | `ORDER CONFIRMATION _25-07_.xlsx`, hash `b99395a9` | P0 | identical bytes → 21 final vs 24 advance |
| F-16 | LOW | `linked_batch_id` is empty on **every** advance document, so advance→final is unmodelled | all advance docs | S0 | linked_batch_id empty on every advance document |
| F-04 · F-05 · F-06 · F-07 · F-08 · F-09 · F-10 · F-11 | — | see `W1_AUTHORITY_MAP.md` | | M0/S1/S3/S4a |
| F-17 | **HIGH** | **Duplicate customer masters are live, not theoretical.** Two legal entities are reported as four positions, so a credit under one id can never offset an invoice under the other. | SYNINVEST/Synalia FR58479048548: cid 38533073 (32 docs, +$43,510.80, −$3,777.00) and cid 38534870 (19 docs, +$18,195.00, −$4,527.00). Goto Jewellery BG202532349: cid 91979891 (31 docs, $219,583.27) and cid 91967768 (1 doc, $13,282.00) | TB-1 (cause) + M0 (the merge itself is operator-only) | 2 legal entities duplicated |
| F-18 | MED | `contractor_detail/id` fallback at `accounting_documents.py:132` and `:211` — the exact wFirma trap: a per-document snapshot id keyed as a contractor | census Z4b | TB-1 ✅ fixed `c7ce03c9` | accounting_documents.py:132 and :211 |
| F-19 | **HIGH** | 13 AR + 2 AP ZOMBIE positions: unpaid invoices AND unlinked credits in one currency. The payment matcher applies payment facts only — there is no correction-to-invoice path, so `correction_of_id` is set by wFirma and ignored by the aggregator | Z2: EUR 4 / PLN 1 / USD 8 AR; USD 2 AP. Railing PLN 557,610 against a PLN 31,297 credit | W4-Z10 decision package | 13 AR + 2 AP zombie positions |
| F-20 | MED | 3 exact-match orphaned credits — the unallocated-pair signature, invoice gross equal to the credit to the cent. The oldest is **four years** old | UAB Tomas Gold $52,940 (2021); Esency Diamonds $9,323.74 (2024); Juliany EOOD $2,843.00 (2026). All carry `correction_of_id` | needs a ruling: refund owed, or write-off | 3 credits; $52,940 (2021); $9,323.74 (2024); $2,843.00 (2026) |
...
| F-38 | **HIGH** | **It is not a CMR bug and it is not two bugs — it is 17 occurrences across 5 modules over four months, and none of them is in an import cycle.** Log census: `accounting_analytics` ×7, `insurance_wording` ×4, `accounting_awb_projection` ×3, `storage_health` ×2, `commercial_document_parties` ×1. Module-level cycle check on all five: **zero cycles; four of them have zero module-level app imports at all**, so a cycle is not merely absent, it is impossible. The lifecycle position is identical in every log: line ~57–61 of a ~60-line stderr file, i.e. the **FIRST ASGI traceback of the process**, after *Application startup complete* and before *Shutting down* — **once per process lifetime**, and the module varies with whichever endpoint is hit first. Structural hazard proven from the graph: four of the five have **zero top-level importers anywhere in the tree** and are reached only by function-scope imports, so their first import always happens inside an anyio worker thread serving a request, never at startup. **The mechanism itself is NOT proven**: two reproductions were built — same-module concurrent import, and a cross-lock pair with opposite lock ordering — and **neither reproduced the error**. Closed as a monitored latent incident with no runtime change. **Reopen when an occurrence is NOT the first ASGI traceback of its process, or when the same endpoint fails on a repeat request** — either would break the once-per-process signature and mean the model here is wrong | 17-occurrence log census; ast module-level cycle check over 5 modules; top-level-importer census; 2 failed reproductions; 5 live 200s | none — outcome B, no code shipped | 17 occurrences; 5 modules; 0 cycles; 4 with 0 top-level importers; 2 failed reproductions |
| F-36 | **HIGH** | **The CMR failure is not a circular import, and the prescribed repair is impossible.** The charter's fix is *"remove the upward dependency from the lower-level party authority"* — that dependency does not exist. Static transitive proof over 36 reachable modules: `commercial_document_parties` has exactly ONE app-level out-edge, `customer_master`, and **does not reach `commercial_cmr` at all**. `resolve_document_parties` has exactly one definition. The only cycle anywhere in the reachable graph is `commercial_authority ↔ proforma_draft_sync`, which is not on the failing edge and is out of scope. Python 3.9's *"(most likely due to a circular import)"* is a **guess appended to the message**, not a diagnosis — and this campaign was scoped from that guess | deployed `commercial_cmr.py` and `commercial_document_parties.py` are byte-identical to main; ast import-graph walk; one definition of the resolver | none — no runtime file was edited | 36 modules reachable; 1 resolver definition; 0 reverse edges; 1 unrelated cycle |
| F-37 | **HIGH** | **The real signature: one failure, on the first request after a restart, and never again.** `C:\PZ\logs\pz_stderr.log` is 5,500 bytes and contains exactly four startup lines, ONE ASGI traceback (the CMR ImportError) and nothing after; mtime 14:50:03, minutes after today's deploy. Called read-only just now, `GET /api/v1/shipment-documents/draft/{id}/cmr.json` returns **http 200 on five different drafts** with a valid document (`seller`/`buyer`/`shipto`/`carrier`/`lines`). The identical *"partially initialized module"* error also appears in the logs for a COMPLETELY different pair — `accounting_analytics.LocalProjectionUnavailable` — which means a **shared mechanism, not two independent cycles**. Leading explanation: concurrent function-scope imports across threadpool workers hitting CPython's import-lock deadlock-avoidance path, which hands back a partially initialized module rather than blocking. A two-thread same-module reproduction was built and **did NOT reproduce it** (CPython serialises that case correctly), so the mechanism needs the cross-lock case and is not deterministically reproducible on demand | production log census; five live endpoint calls; a purpose-built reproduction that failed to reproduce | open — repair proposal below, not yet implemented | 1 failure; 5/5 live 200s; 2 unrelated modules, same error text |
