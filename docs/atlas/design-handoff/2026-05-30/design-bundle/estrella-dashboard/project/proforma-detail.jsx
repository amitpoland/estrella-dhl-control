...
function ProformaDetailPage({ draft, onBack, onConvert }) {
  ...
  const detail = {
    ...
    customer: {
      name: draft.customer,
      vatEu: 'NL123456789B01',
      address: 'Diamantweg 12, 1234 AB Amsterdam',
      country: 'Netherlands',
      wfirmaId: draft.status === 'convert_blocked' ? null : 'WF-CUST-4421',
      wfirmaName: draft.status === 'convert_blocked' ? null : 'Diamond Point NV',
    },
    exporter: {
  ...
  const canConvert = draft.status === 'ready';
  const isBlocked = draft.status === 'convert_blocked';
