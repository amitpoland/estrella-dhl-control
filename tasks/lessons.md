...
### L-050 — Mechanical-equivalent smoke is a legitimate sub-result; never claim "browser smoke complete" without a real session
- **Evidence:** B-MD4 was authored as an operator-driven 20-surface browser walk. When run by an agent without a browser session, ~14 of 20 surfaces could be mechanically verified (testid presence in deployed file + GET endpoint reachability + temp-record CRUD round-trip via curl). The remaining 6 surfaces (§2 Customer Master + §3 Shipping Addresses + §4 Carrier Accounts + §5 KYC + §6 KUKE/Credit + §7 Invoice Settings + §18 AdminUsersPage UI walk) genuinely require an admin browser session to verify visual rendering of session-gated routes (`/dashboard/{path}` requires `check_session_or_redirect`). The smoke report explicitly distinguishes "mechanically verified PASS" from "operator-only deferred" rather than conflating them.
- **Rule:** When an automated agent runs a checklist intended for an operator browser session, the result must explicitly partition the surfaces into (a) "mechanically verified" with the specific mechanical evidence (testid presence + API round-trip + storage delta), and (b) "operator-only deferred" with a concrete reason (session-gated route / destructive action on real record). Do NOT mark a batch fully `smoked` if any operator-only surface remains unverified — instead mark it `smoked` with an explicit operator-only continuation footnote, and pause the campaign until the operator completes the deferred walks. Never elide the distinction; future operators trusting a "smoked" label without that distinction is a real risk.

### L-049 — Browser smoke checklists must enumerate temp records + destructive avoidance up front
- **Evidence:** B-MD4 has 20 surfaces, several of which support write actions (Suppliers, Designs, Product-local, Customer Master, AdminUsersPage). Without an explicit naming convention for temp smoke records (`SMOKE_MD4_DSGN_001`, `SMOKE_MD4_SUP_001`, `SMOKE-MD4-PL`) and an explicit "destructive action AVOIDED" cell per surface, an operator running through the checklist might mutate a real production record by accident. The B-MD4 checklist makes both rules per-surface explicit and includes a final cleanup-verification step.
- **Rule:** Every multi-surface browser smoke checklist for production must (a) reserve a documented temp-record naming convention for write surfaces, (b) include a "destructive action AVOIDED" cell that names the specific real-data write the operator must NOT perform, and (c) end with a cleanup-verification step that proves every temp record was deleted. Without all three, the smoke checklist itself is a risk surface.

### L-048 — Orphan components are technical debt; remove them when their last caller leaves
- **Evidence:** The `PendingPanel` reusable component had 2 callers in MDC-era code: `activeEntity === 'designs'` and `activeEntity === 'roles'`. B-MD2 replaced both call sites with live components. The PendingPanel definition was left behind, dead code: zero `<PendingPanel ...` usages anywhere, but the function definition + its disabled `+ New {title}` / `Import CSV` buttons + its `Backend pending` badge still occupied 45 lines and confused source-grep cleanup audits. B-MD3 removed the orphan and replaced it with a comment recording the removal.
- **Rule:** When a refactor (here B-MD2) removes the last caller of a reusable component, the SAME PR (or the immediate follow-up cleanup PR) should remove the component definition. Leaving it as dead code drags through source-grep audits and triggers false-positive "Backend pending" matches that mask real stale labels elsewhere. Acceptable shortcut: if the cleanup is non-trivial, leave a TODO comment + open a follow-up issue/batch (B-MD3 is exactly this pattern).

### L-047 — Lazy-init SQLite tables grow only on first use
- **Evidence:** B-MD2 deploy added a `designs` table to `master_data.sqlite::init_db`. Before the first `/api/v1/designs/` request, the file size was unchanged. After the first GET (which triggers `init_db(_DB_PATH)`), the file grew from 90,112 → 114,688 bytes (+24,576 B for the table + 4 indexes). The smoke report records both states and explains the delta. Operators inspecting `master_data.sqlite` size pre-vs-post deploy must wait for the first authenticated GET on the new endpoint, or trigger it as part of the smoke battery, before comparing.
- **Rule:** When deploying an additive `CREATE TABLE IF NOT EXISTS` via lazy `init_db`, the smoke report must document the file-size delta in two stages: (a) immediately after deploy (file size unchanged, table not yet created), and (b) after the first endpoint hit (file size grows by table + indexes). Operators reading the smoke report later understand that a "size unchanged" measurement immediately post-deploy is correct, not a defect.

### L-046 — Soft references (no SQL FK) keep additive masters non-destructive
- **Evidence:** B-MD2 Designs master ships with `product_ref`, `hs_code`, and `unit` columns that conceptually reference other tables (wFirma products, `hs_codes`, `units`). The DDL deliberately does NOT include `FOREIGN KEY ... REFERENCES ...` clauses. Verified by `test_b_md2_designs_table_has_no_fk_constraints` (PRAGMA `foreign_key_list(designs)` returns empty). This is consistent with the rest of the master-data style: a stale `hs_code` value on a design does not cascade-delete or refuse-update; the operator UI explains the soft reference and the consumer code resolves it at read time.
- **Rule:** Additive local-SQLite masters that cross-reference other tables MUST use soft references (FK-by-value, no SQL constraint). Two reasons: (a) SQL FK constraints turn benign operator edits into destructive cascades when the referenced row is later renamed or removed; (b) keeping the constraint at the application layer makes it possible to ship the dependent master before its referenced master is fully complete. Pin the absence of FK with a `PRAGMA foreign_key_list` test on each new master table.
...
### L-040 — Router file name is NOT its mount prefix
- **Evidence:** PR #141 (B0 wFirma identity cache, merge SHA 08529b3) shipped frontend buttons and test allow-lists hard-coding `/api/v1/wfirma/capabilities/customers/...` because the route file is named `routes_wfirma_capabilities.py`. The actual router declares `APIRouter(prefix="/api/v1/wfirma")`. All unit tests passed (they only source-grep, not HTTP). Phase-5 production smoke caught a 404 on the customer Apply call, requiring hotfix PR #142 (ad82ab6) to redeploy `dashboard.html` plus three test allow-lists.
- **Rule:** Whenever a frontend button or test allow-list references a URL, the test step that follows MUST be a live route resolution check (FastAPI `app.routes`, or a 200/expected response against the running app) — not just a source-grep contract. Source-grep tests prove the *literal string* is present; they do NOT prove the string is a valid URL. Add a contract test that walks `dashboard.html` for every `/api/v1/...` literal and asserts each one resolves to a registered route on the real app instance.
- **Mitigation in this codebase:** the existing `test_master_data_hard_rules.py` `allowed_prefixes` test is structural only. A new `test_dashboard_urls_resolve_against_app.py` would close this gap by importing the FastAPI app and asserting every dashboard URL literal matches an `app.routes` entry.

### L-041 — wFirma ceiling: residual missing fields are not always actionable
