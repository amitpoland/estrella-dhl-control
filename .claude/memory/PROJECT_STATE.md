...
## Current origin/main HEAD (2026-06-20, updated): `8241abd`

- **origin/main HEAD = `8241abd`** — `Add AUTHORITY_MAP.md — workflow ownership single source of truth (#660)` (merged 2026-06-20 squash). Supersedes the `0574270` HEAD block below (append-only — prior entries retained).
- **Production functional SHA**: last operator-verified deploy = `03ffce9` (#652). PRs #659, #660 and earlier undeployed merges (#645, #640, #643, #647, #648 region, #652–#658 chain) are on main but NOT yet synced to `C:\PZ`.

## PR #660 — AUTHORITY_MAP.md + TASK_STATE.md initial (2026-06-20, MERGED as `8241abd` — NOT DEPLOYED)

- **PR #660 MERGED** (2026-06-20): Merged to `origin/main` as squash `8241abd`. Title: `Add AUTHORITY_MAP.md — workflow ownership single source of truth (#660)`.
- **Scope**: Governance/docs-only. Two artifacts added: (1) `docs/governance/AUTHORITY_MAP.md` — 500-line document naming the authoritative system for each of 9 workflow domains (Proforma, DHL, PZ, Customer Master, Product Master, Shipment Master, Inventory, Audit/Evidence, wFirma). (2) `.claude/memory/TASK_STATE.md` — initial file; ephemeral single-task state tracker.
- **Purpose**: Single source of truth for workflow ownership across all 9 domains. Establishes the AUTHORITY_MAP as the governance reference for any future code that crosses domain boundaries.
- **GATE 2 impact**: Governance/docs-only class — does NOT consume an implementation PR slot. Zero blast radius; no `service/app/**` files touched; no robocopy required.
- **Lesson J N/A**: no root engine files touched. No `C:\PZ\engine\` sync required.

## PR #659 — Anti-HOLD Governance Package (2026-06-20, MERGED as `b737fdc` — NOT DEPLOYED)

- **PR #659 MERGED** (2026-06-20): Merged to `origin/main` as squash `b737fdc`. Title: `docs(governance): add Anti-HOLD and Workflow Completion governance`.
- **Scope**: Governance/docs-only. Three changes: (1) `CLAUDE.md` — added §ANTI-HOLD AND WORKFLOW COMPLETION section (4 HOLD conditions, 6 must-continue classes, completion checklist, TASK_STATE/PROJECT_STATE boundary); (2) `docs/governance/anti-hold-and-completion.md` — full governance spec; (3) `.claude/memory/TASK_STATE.md` — status set to COMPLETE.
- **Purpose**: Anti-HOLD governance prevents tasks from stalling without explicit operator decision. Defines when a task may and may not be held. Establishes TASK_STATE.md as ephemeral single-task tracker (distinct from PROJECT_STATE.md which is cross-session project memory).
- **GATE 2 impact**: Governance/docs-only class — does NOT consume an implementation PR slot. Zero blast radius; no `service/app/**` files touched.
- **FIRST COMPLETE GOVERNANCE PACKAGE** (2026-06-20): PRs #659 + #660 together form the first complete governance package: Anti-HOLD + TASK_STATE + AUTHORITY_MAP. All three artifacts are now on `origin/main`.

## Current origin/main HEAD (2026-06-19, updated): `0574270`

- **origin/main HEAD = `0574270014afaa0daa8868ddef6e79dca3cbdc38`** — `feat(governance): Claude Code governance enforcement layer — session-start hook, gap-detection hook, pre-commit duplicate-module guard (#657)` (merged 2026-06-19 squash). Supersedes the `03ffce9` HEAD block below (append-only — prior entries retained).
- **Production functional SHA**: last operator-verified deploy = `03ffce9` (#652). PRs #645 (#657) and earlier undeployed merges (#640, #643, #647, #648 region, #652–#657 chain) are on main but NOT yet synced to `C:\PZ`.

## PR #657 — Claude Code Governance Enforcement Layer (2026-06-19, MERGED as `0574270` — NOT DEPLOYED)

- **PR #657 MERGED** (2026-06-19): Merged to `origin/main` as squash `0574270014afaa0daa8868ddef6e79dca3cbdc38`. Title: `feat(governance): Claude Code governance enforcement layer — session-start hook, gap-detection hook, pre-commit duplicate-module guard`.
- **Governance/zero-blast-radius class** — does NOT consume an implementation PR slot (GATE 2 exemption). No files in `service/app/**` touched; no robocopy required; no production service restart.
- **Three artifacts added**:
  1. `hooks/pre-commit` — duplicate-module guard: blocks any staged NEW file whose basename already exists elsewhere in `service/app/**` under a different path. Exempt: `__init__.py`, `conftest.py`, `.gitkeep`, `test_*.py`, `components.js/.jsx`. Override: `git commit --no-verify` (must disclose).
  2. `.claude/hooks/pz-session-start.py` — `UserPromptSubmit` hook: injects PROJECT_STATE.md summary (date, GATE 2 state, top 5 OPEN QUESTIONS) on the first user message per session (once-per-session `/tmp/` marker prevents repeated injections).
  3. `.claude/hooks/pz-gap-detection.py` — `SessionStart` hook: structural-risk scanner (duplicate modules, files outside approved root, parallel app structures, stale PROJECT_STATE.md >5 days). ANTI-HOLD: never blocks, advisory only, fails open on any error.
- **`.claude/settings.json` updated**: `pz-gap-detection.py` added to `SessionStart` hooks; new `UserPromptSubmit` event registered with `pz-session-start.py`.
- **Lesson B caveat** (2026-06-19): mid-session `settings.json` changes are NOT guaranteed live. These hooks register on the NEXT fresh Claude Code session, not the current one.
- **Lesson J N/A**: all files are in `.claude/hooks/` and `hooks/` repo-root — outside `service/app/**`; no `C:\PZ\engine\` sync required. Not deployed via robocopy.
- **Deploy note**: hooks live in the repo working tree (`.claude/hooks/`, `hooks/`). On Windows production host (`C:\PZ-verify`), a `git pull` is sufficient to activate them. No service restart required.
- **GATE 2 impact**: governance-only class, does NOT decrement implementation PR slot count. Current impl slots post-merge: unchanged (see below).

## PR #645 — V2 Blank-Screen Fix (2026-06-19, MERGED as `80b2e08` — NOT YET DEPLOYED to C:\PZ)

- **PR #645 MERGED** (2026-06-19): Merged to `origin/main` as squash `80b2e08`. Title: `fix(v2): serve .jsx as text/javascript, vendor-first CDN fallback, 404 for missing assets`.
- **Scope**: Frontend-only. Three fixes: (1) `.jsx` files served as `text/javascript` MIME type instead of `application/octet-stream` (was causing blank V2 pages); (2) vendor-first CDN fallback strategy for React/Babel bundles; (3) correct 404 response for missing static assets instead of serving an error page.
- **Deploy requirement**: Operator must execute on Windows server: (a) standard robocopy `service/app → C:\PZ\app` + (b) run `download-v2-vendor.ps1` to pull updated vendor bundles. PZService restart required. No Lesson J engine files touched.
- **GATE 2 impact**: This was previously counted as an open docs/CLAUDE.md PR slot. Post-merge: freed one slot. Current GATE 2 state: **implementation PRs open = #630 (proforma governance) + #647 (vision-invoice-confirm PR-2)** = **2/3 slots used**. One slot available.
...
## Claude Code Governance Hooks — Architecture Decisions (2026-06-19, PR #657)

- **Duplicate-module guard (`hooks/pre-commit`) is the enforcement mechanism for module-duplication prevention** (2026-06-19): any staged NEW file whose basename already exists elsewhere in `service/app/**` under a different path is blocked at commit time. Override requires `--no-verify` with disclosed reason. Exempt files: `__init__.py`, `conftest.py`, `.gitkeep`, `test_*.py`, `components.js/.jsx`.
- **`pz-gap-detection.py` (SessionStart) is ANTI-HOLD by design** (2026-06-19): the hook MUST fail open — any exception or missing file causes it to silently exit 0 so it never blocks a session. Advisory output only; no session-blocking.
- **Hooks activate on NEXT fresh session** (2026-06-19, Lesson B): changes to `.claude/settings.json` mid-session are not guaranteed live. The session that merged PR #657 does NOT see the new hooks. The operator must start a fresh Claude Code session to activate them.
- **Windows production host activation**: `git pull` in `C:\PZ-verify` is sufficient — no service restart. The hooks are repo-resident (`.claude/hooks/`, `hooks/`), not part of the `service/app → C:\PZ\app` robocopy surface.

## First Complete Governance Package — Architecture Decisions (2026-06-20, PRs #659 + #660)

- **AUTHORITY_MAP.md is the workflow ownership source of truth** (2026-06-20): `docs/governance/AUTHORITY_MAP.md` names the authoritative system for each of 9 workflow domains. Any code that crosses domain boundaries must reference this map; any new domain must be added here before implementation begins.
- **TASK_STATE.md is an ephemeral single-task tracker** (2026-06-20): `.claude/memory/TASK_STATE.md` tracks the current in-flight task only — cleared on task completion. It is distinct from PROJECT_STATE.md (which is cross-session permanent project memory). The TASK_STATE/PROJECT_STATE boundary is defined in `docs/governance/anti-hold-and-completion.md`.
- **Anti-HOLD governance is binding on all implementation work** (2026-06-20): `CLAUDE.md` §ANTI-HOLD AND WORKFLOW COMPLETION defines 4 HOLD conditions (operator approval needed, external dependency, explicit gate block, data safety risk) and 6 must-continue classes (investigation, GATE 1 pre-flight, GATE 4 disposition, memory/state update, test execution, reviewer dispatch). A task may only HOLD on one of the 4 conditions; all other pauses are workflow violations. Full spec: `docs/governance/anti-hold-and-completion.md`.

## Next 3 actions in queue (refreshed 2026-06-20 — PRs #659 + #660 MERGED; GATE 2 now 0/3)

1. **Operator: deploy PR #645 V2 blank-screen fix to C:\PZ** — target: V2 pages load without blank screen on production. Steps: (a) robocopy `service/app → C:\PZ\app`, (b) run `download-v2-vendor.ps1` on Windows server, (c) PZService restart. Gating: operator-only (deploy-guard hook). No 7-agent deploy gate required for frontend-only static asset change, but recommend smoke-verify V2 page loads post-restart.
2. **Confirm status of PR #647** (branch `feat/pr2-vision-invoice-confirm-workflow` @ `4429e04`; Stage B operator-confirm for image-only invoice) — verify whether still open or merged since last update. If still open: GATE 2 is 0/3 so one impl slot free. After merge + deploy, AWB 2315714531 operator can confirm via `POST /dashboard/batches/{id}/vision-invoice/confirm`. Gating: GATE 2 (board currently clear); Issue #646 ratification (recheck_batch lost-update hazard — must decide BLOCKING vs POST-DEPLOY).
3. **Confirm status of PR #630** (branch `feat/pr1a-conflict-foundation-remediation`; proforma governance remediation) — verify whether still open or merged since last update. If still open: start merge review; GATE 2 allows it with current 0/3 count. Gating: GATE 2 (0/3 — board clear); full test suite pass required before open.
