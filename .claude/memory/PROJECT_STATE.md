...
## PR #640 — Advisory Image-Only Invoice Extraction (PR-1) (2026-06-17, OPEN — MERGEABLE — NOT DEPLOYED)

- **#632 / #633 MERGED to origin/main** (VERIFIED via `gh pr view` + `git log origin/main`): both `state=MERGED`. `origin/main` HEAD = **`4652292`** (`#633` merge commit `4652292`; `#632` merge commit `c284902`). This supersedes the prior FACTS note that listed #632/#633 as OPEN. #632 = OCR/AI vision **CIF** fallback for image-only customs docs; #633 = UI + Polish-desc gate read resolved CIF authority.
- **Production deploy of #632/#633 — OPERATOR-REPORTED, NOT INDEPENDENTLY VERIFIED.** Operator states production is at `4652292` and #632/#633 are deployed. Could NOT be confirmed from git: `C:\PZ` is robocopy-synced, not a git checkout, so no deployed-SHA readout is possible there. Prior recorded production HEAD was `e4d96b5`. **To promote to a hard FACT:** LF-normalized SHA256 file-hash check on `C:\PZ-verify` @ `4652292` vs `C:\PZ` (per the authority-hash EOL-normalization rule), or release-manager deploy record. Until then treat "production = 4652292" as reported, not verified.
- **PR #640 OPEN + MERGEABLE** (VERIFIED): branch `fix/invoice-image-only-lineitem-extraction`, HEAD `1a2689f` (code at `f6c7ec2`, docs at `d04e95c`/`1a2689f`). Base/merge-base = `origin/main` `4652292` (clean; branch adds exactly its own commits). Title: `fix(invoice): advisory image-only invoice extraction (FOB + line items + supplier) — PR-1`. `mergeable=MERGEABLE`, not draft. Reviewer-challenge PASS (4 REQUIRED findings resolved at `f6c7ec2`). **NOT merged, NOT deployed.**
...
## PR #643 — Single Resolved-CIF Authority Backend Guard (2026-06-17, OPEN — NOT DEPLOYED)

- **PR #643 OPEN** (2026-06-17): Branch `feat/cif-authority-consistency-guard`, commit `20d6a0c`. Title: `feat(customs): single resolved-CIF authority backend guard — require_resolved_cif across all action gates`. Base: `main`. NOT deployed. NOT merged. GATE 1 satisfied. GATE 2: opened with 1 free slot (open PRs at open time: #637 docs-only, #630 impl → 2/3 used of impl limit; slot confirmed free). https://github.com/amitpoland/estrella-dhl-control/pull/643
...
## Next 3 actions in queue (refreshed 2026-06-17 — PR #643 CIF-authority-consistency-guard OPENED)

1. **Operator: review + merge PR #643** (branch `feat/cif-authority-consistency-guard` @ `20d6a0c`; GATE 1 satisfied; all 5 reviewers EXEMPLARY; 73 targeted tests pass; zero new baseline failures). Then run 7-agent deploy gate (standard service/app robocopy, no Lesson J engine files; new file `cif_authority.py` lands cleanly) and deploy to `C:\PZ`. Verify AWB 2315714531 clearance no longer returns `total_value_usd=0.0`. Gating: GATE 2 has 1 slot remaining (2/3 used: #630 impl + #637 docs).
2. **Operator: review + merge PR #633** (CIF-UI resolved-authority, branch `fix/cif-ui-resolved-authority` @ `49f1060`; GATE 1 satisfied; 3 reviewers EXEMPLARY). Standard service/app + `shipment-detail.html` sync. Gating: 1 GATE 2 impl slot; may sequence before or after #643 (no code dependencies between them at service layer).
3. **Dispose of pre-existing test-coverage-reviewer weak-verdict pattern** (repeated severity-inflation signal from 2026-06-12 scorecard; 4+ occurrences; still open — no direct dispatch to assess calibration has been made). Target: file as ISSUE or SCHEDULED before next implementation campaign that uses test-coverage-reviewer. Gating: operator or next session can self-resolve.
...
## OQ-PR643-MERGE: PR #643 awaiting merge gate + deploy (2026-06-17)

- **Question / requirement**: PR #643 (single resolved-CIF authority backend guard, branch `feat/cif-authority-consistency-guard` @ `20d6a0c`) is OPEN and GATE 1 satisfied. It requires operator review, merge, 7-agent deploy gate execution, and deploy to `C:\PZ` before it is live.
- **Who can answer / close**: Operator — merge via `gh pr merge 643 --squash --delete-branch`; run 7-agent gate against clean worktree at merged SHA; execute standard service/app robocopy (no Lesson J engine files); verify prod hash flips; confirm AWB 2315714531 no longer returns `total_value_usd=0.0`. Closes when production deploy verified.
- **Impact if deferred**: The six action-gate call sites (`routes_dhl_clearance`, `routes_dsk`, `routes_agency`, `routes_action_proposals`, `routes_dashboard`) continue reading raw invoice CIF instead of the resolved tri-state authority — AWB 2315714531 class shipments remain false-blocked at the description/DSK/routing gates.
- **GATE 2 state at open time**: 2/3 impl slots used (#630 impl + #637 docs); 1 slot available for #643. Closing #637 (docs) frees a docs-class slot; closing #630 frees an impl slot. PR #643 may merge as the 2nd impl PR if #630 is still open at merge time.
