...
Owned by `flow-context-keeper`. Do not edit by hand outside of an emergency. Last updated on 2026-06-15 (Campaign 03 Sprint 03.2 PR #608 merged to main as `1909fcc` — Shipment Detail V2 authority-honest UX polish, MERGED-NOT-DEPLOYED; origin/main HEAD now **1909fcc**; **PRODUCTION REMAINS d37316e** — neither #608 nor #602/f8108ae deployed yet; GATE 2 now **0/3 open impl PRs**; NOTE: `flow-context-keeper` is non-dispatchable this session — pinned to unavailable model `claude-sonnet-4-20250514` — so this update was hand-written under the emergency clause; see #608 FACT). Prior: PR #602 resolves Issue #598; docs PRs #603/#605/#607 merged; #604 closed unmerged.

**Last-run-at:** 2026-06-15 (Campaign 03 Sprint 03.2 PR #608 merged as `1909fcc` — Shipment Detail V2 authority-honest UX polish; prior same-day: PR #602 resolves Issue #598; PR #599 test-only; docs PRs #603/#605/#607; #604 closed unmerged). Origin/main HEAD: **1909fcc** (feat(v2): authority-honest UX polish for Shipment Detail V2 (Sprint 03.2) (#608) on top of 65f5533). Production still **d37316e** — #608, #602/f8108ae all merged-not-deployed. GATE 2: **0/3 open impl PRs** (#498 draft only, not counted active). TEST BASELINE: 221/221 PZ regression + 412/412 carrier suite (per `.claude/contracts/test-baseline.md`). DHL AUTOMATION: dev-phase flows ENABLED (shadow_mode=false, 5 AUTO_* flags true, all AUTO_SEND_* false). PROFORMA: **Write Enablement Phase 1A+1B MERGED** — Edit/Cancel Draft/Prior Invoices/Send Email enabled; CMR/Generate remain disabled with reasons (Lesson M). **M2 SEND: FUNCTIONALLY COMPLETE** — full pipeline verified including PDF fetch; SMTP path deferred to natural workflow. ATLAS-V2: **WIRED_PAGES = 17/17 (100%)** — ALL V2 pages authority-honest, MOCK banner retired. COMPLIANCE RESOLVER: LIVE (COMPLIANCE_INTELLIGENCE_RESOLVER_ENABLED=true). **PYCACHE RULE**: Backend deploys to C:\PZ must clear ALL __pycache__ recursively (app + engine) before restart — `Get-ChildItem -Path C:\PZ -Recurse -Filter __pycache__ | Remove-Item -Recurse -Force` — else stale .pyc shadows new source silently. **EXCEL COLUMN MAPPING**: Advisory endpoint live (suggest-column-mapping), supplier template approval framework deployed, LLM safety gates enforced (operator_confirmed required). **M6 PRIOR PROFORMA SEARCH**: **CAMPAIGN CLOSED** (2026-06-08). **CUSTOMER MASTER ADDRESS AUTHORITY**: **CAMPAIGN CLOSED** (2026-06-07).
...
- **RULE 3 bookkeeping note**: this FACT was written by hand — `flow-context-keeper` is currently NON-DISPATCHABLE (its agent frontmatter pins the unavailable model `claude-sonnet-4-20250514`; both a plain dispatch and a `model: sonnet` override failed). Hand-edit permitted per the line-5 emergency clause. The broken-model agent is disclosed as a follow-up registry/model-repair finding (GATE 5 / Lesson-B-adjacent). This PROJECT_STATE edit is being carried on a docs-only PR branch per the memory-commit rule; no direct-to-main chore(memory) push.
...
1. **Issue #597 environment-disclosure-gap** — target: resolve observer self-degradation on environment disclosure gaps — gating: OPEN issue from self-eval-2026-06-15.md requires action
2. **PR #498 draft completion** — target: operator decision to advance security draft from draft status or close — gating: GATE 4 H-R5/H-W3/H-W2 findings require disposition (security: H-R5 admin role guard, H-W3 session-bound actor)
3. **Deploy f8108ae** — target: deploy merged PR #602 wFirma dead import cleanup to production — gating: deferrable low-risk, production remains d37316e, awaiting next deploy window
...
## OQ: flow-context-keeper non-dispatchable — pinned to unavailable model (2026-06-15)

- **Question**: How to repair `flow-context-keeper` so RULE 3 PROJECT_STATE updates can run via the agent again instead of by-hand.
- **Detail**: The agent frontmatter pins model `claude-sonnet-4-20250514`, which is not available this session — both a plain dispatch and a `model: sonnet` override failed to dispatch. The #608 RULE 3 update (this entry's neighbour FACT) was therefore hand-written under the line-5 emergency clause. This is a GATE 5 / Lesson-B-adjacent registry/model-repair finding (silent meta-agent substitution is forbidden — disclosed instead).
- **Who can answer**: Operator or agent-prompt-refiner (when deployed) — repoint `.claude/agents/flow-context-keeper.md` frontmatter to an available model.
- **Impact if unanswered**: Every future PR-merge / issue-close RULE 3 update must be hand-written (emergency clause), raising drift risk and bypassing the agent's movement-rule matrix (FACTS append-only invariant enforced manually).
