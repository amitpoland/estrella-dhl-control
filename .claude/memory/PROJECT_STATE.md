...
## PR #627 — CIF Tri-State Authority Resolver (2026-06-16, OPEN — awaiting review/merge)
...
1. **Merge PR #627** (CIF tri-state resolver) — target: operator review + merge approval; gating: reviewer-challenge verdict already EXEMPLARY (GATE 1 satisfied); deploy is a separate operator step after merge, requires 7-agent gate due to financial-adjacent scope.
2. **Clear one impl PR slot** (merge/close at least one of #626 or #625) — target: free a slot before any 4th impl PR can open; gating: GATE 2 at 3/3 ceiling.
...
## OQ-PR627: PR #627 CIF tri-state resolver — awaiting review/merge; deploy gated on operator approval (2026-06-16)
...
- **Who can answer**: Operator — merge decision; then a separate operator 7-agent `/deploy` approval for the production sync (customs/financial-adjacent scope means no autonomous deploy is permitted).
- **Impact if left unanswered**: (1) AWB 2315714531 and any future shipment where invoice CIF never landed continues to see `total_value_usd=0.0` in clearance decisions rather than the correct USD 732. (2) The FedEx clearance path retains its pre-fix silent `float(... or 0)` coercion. (3) The `shipment-detail.html` Clearance Routing card continues to display "USD 0.00" for genuine extraction gaps rather than "Not calculated."
- **Preconditions for deploy**: PR #627 merged to main + full 7-agent `/deploy` gate must run (backend-impact + security + persistence reviewers required given CIF is a financial calculation input). No deploy may occur without explicit operator go-ahead.
- **GATE 2 note**: 3/3 impl PRs open (#626, #625, #627); operator must clear at least 1 before a 4th impl PR may open.
- **Candidate closure path**: operator merges PR #627 → 7-agent gate → deploy → verify `GET /api/v1/shipment/<awb>/clearance` no longer returns `total_value_usd=0.0` for AWB 2315714531 → OQ closes and moves to FACTS.
