...
Owned by `flow-context-keeper`. Do not edit by hand outside of an emergency. Last updated on 2026-06-17 (PR #632 OCR/AI image-only extraction fallback OPEN — branch feat/ocr-ai-image-only-extraction-fallback @ eca52c7; 8 files +1829; 42 tests pass; GATE 1 satisfied; scorecard 2026-06-17-ocr-ai-image-only-extraction-fallback.md; NOT deployed; vision-LLM fallback wired into 6-layer CIF authority ladder with no resolver edit required; GATE 4 SCHEDULED: security-permissions → deploy-security-reviewer swap + backend-safety-reviewer re-run on vision write paths). Prior (2026-06-16): PRs #625/#627/#631/#628 MERGED — CIF tri-state resolver + AWB robust extraction + e2e regression + memory; origin/main HEAD now a421fe9. Prior (2026-06-16): ADR-029 PR-1 #626 MERGED to main as `d80a816` — conflict-detection foundation, all flags OFF, zero blast radius; scorecard 2026-06-16-adr029-pr1-conflict-foundation.md. Prior (2026-06-16): E3b DEPLOYED to production at `C:\PZ`; `inbox-page.jsx` LF-normalized sha256 6fcfcd2fa6bb02c2a98db5f00dde0f1cd6cebf9a1c16d8383e73956add9d6bac verified; deploy SHA origin/main=92fe65b; 7-agent gate ALL CLEAR; PZService alive. Prior (2026-06-16): E3a DEPLOYED to production at `C:\PZ`; `routes_inbox.py` SHA-256 69CB229A… hash-verified; PZService RUNNING; E3b now UNBLOCKED. Prior (2026-06-16): PR #614 MERGED to main at `178a392` — Sprint 03.3 Scope C E3a inbox evidence endpoint on main, NOT YET DEPLOYED. Prior (2026-06-16): PR #614 OPEN → MERGED. Prior (2026-06-15): PR #608 merged as `1909fcc`, PR #602 resolves Issue #598.

**Last-run-at:** 2026-06-17 (PR #632 OCR/AI image-only extraction fallback OPEN — branch `feat/ocr-ai-image-only-extraction-fallback` @ `eca52c7`; 8 files +1829; 42 tests pass; GATE 1 satisfied; scorecard `.claude/memory/scorecards/2026-06-17-ocr-ai-image-only-extraction-fallback.md`; NOT deployed; vision-LLM fallback over fitz-rasterized PNGs; vision writes into existing 6-layer CIF authority ladder — no resolver edit; EV_VISION_CIF_WRITTEN timeline event added; dhl_precheck fob_total_usd/vision_extracted/vision_source_page preservation guard closes #570-class C1 CRITICAL; `shipment-detail.html` authority-honest status InfoRow `data-testid="clearance-extraction-method"` added). Prior (2026-06-16): PRs #625/#627/#631/#628 MERGED to origin/main — CIF tri-state resolver (`e4d96b5`), AWB robust extraction (`729afe2`), e2e tri-state regression + merge-not-downgrade fix (`a421fe9`), memory docs (`2a3a117`); origin/main HEAD **a421fe9**. Prior (2026-06-16): ADR-029 PR-1 #626 MERGED as `d80a816` — conflict-detection foundation; 2 commits c25af76+141dd0d; 66 conflict tests green; GATE 1 CLEAR; scorecard `.claude/memory/scorecards/2026-06-16-adr029-pr1-conflict-foundation.md`. Prior (2026-06-16): PR #621 E3b EvidencePanel DEPLOYED to production — deploy SHA `92fe65b`; `inbox-page.jsx` 453→743 lines robocopy'd; LF-normalized sha256 `6fcfcd2fa6bb02c2a98db5f00dde0f1cd6cebf9a1c16d8383e73956add9d6bac` verified; 7-agent gate 6 CLEAR + lead-coordinator READY-TO-DEPLOY; PZ 221/221, carrier 420/412, E3b panel 14/14; backup retained; service alive auth-gated 401. Prior (2026-06-16): PR #621 MERGED to main as `2144c0b`. Prior (2026-06-16): E3a DEPLOYED — 7-agent gate ran; `routes_inbox.py` SHA-256 `69CB229A…` verified; PZService RUNNING process 9840 on 127.0.0.1:47213; E3b UNBLOCKED. Origin/main HEAD: **a421fe9** (test: CIF tri-state e2e regression + merge-not-downgrade gap poisoning fix, PR #631). Production: **E3b DEPLOYED** — `inbox-page.jsx` live at `C:\PZ\app\static\v2\`; prior production bundle at `2144c0b` (E3a+E3b+#608+#602+#614 all live); #625/#627/#631 on main NOT YET DEPLOYED to C:\PZ. GATE 2: 2 open implementation PRs (#632 OCR/AI fallback, #630 conflict-foundation remediation); +0 docs. TEST BASELINE: 221/221 PZ regression + 412/412 carrier suite (per `.claude/contracts/test-baseline.md`). DHL AUTOMATION: dev-phase flows ENABLED (shadow_mode=false, 5 AUTO_* flags true, all AUTO_SEND_* false). PROFORMA: **Write Enablement Phase 1A+1B MERGED** — Edit/Cancel Draft/Prior Invoices/Send Email enabled; CMR/Generate remain disabled with reasons (Lesson M). **M2 SEND: FUNCTIONALLY COMPLETE** — full pipeline verified including PDF fetch; SMTP path deferred to natural workflow. ATLAS-V2: **WIRED_PAGES = 17/17 (100%)** — ALL V2 pages authority-honest, MOCK banner retired. COMPLIANCE RESOLVER: LIVE (COMPLIANCE_INTELLIGENCE_RESOLVER_ENABLED=true). **PYCACHE RULE**: Backend deploys to C:\PZ must clear ALL __pycache__ recursively (app + engine) before restart — `Get-ChildItem -Path C:\PZ -Recurse -Filter __pycache__ | Remove-Item -Recurse -Force` — else stale .pyc shadows new source silently. **EXCEL COLUMN MAPPING**: Advisory endpoint live (suggest-column-mapping), supplier template approval framework deployed, LLM safety gates enforced (operator_confirmed required). **M6 PRIOR PROFORMA SEARCH**: **CAMPAIGN CLOSED** (2026-06-08). **CUSTOMER MASTER ADDRESS AUTHORITY**: **CAMPAIGN CLOSED** (2026-06-07). **ADR-029 CONFLICT DETECTION**: PR-1 MERGED (#626, `d80a816`) — 4 validators wired (V3/V4/V5/V8), all 4 flags default OFF, zero blast radius; PR-2 scope = V1/V2/V6/V7 detectors + §5 hard gate + list_draft_conflicts 404 fix. **CIF AUTHORITY**: tri-state resolver MERGED (#627, `e4d96b5`) + AWB extraction hardening (#625, `729afe2`) + e2e regression (#631, `a421fe9`) — all on main, NOT YET DEPLOYED to C:\PZ. **OCR/AI VISION FALLBACK**: PR #632 OPEN — NOT deployed; production deploy requires explicit operator approval + full 7-agent gate; wFirma/SAD/ZC429/VAT/deploy-scripts untouched.
...
## PR #626 — ADR-029 PR-1: Conflict Detection Foundation (2026-06-16, MERGED as `d80a816`) — BACKEND ONLY, NOT DEPLOYED

- **PR #626 MERGED** (2026-06-16): title `feat(proforma): ADR-029 PR-1 — conflict-detection foundation (flags OFF)`. Branch `feat/adr-029-pr1-conflict-foundation`. Base: `main`. Merged to main as `d80a816`. **NOT deployed.** Two commits: `c25af76` (conflict-detection foundation — 8 files, 1919 insertions) + `141dd0d` (backend-safety fixes — 3 files).
...
- **Deploy status**: NOT DEPLOYED — customs/financial-adjacent change requires explicit operator approval + reviewer-challenge before any deploy. No deployment performed.
- **GATE 2 after PR #627 open**: #626 now MERGED; merge order = **#625 first, then #627**, single deploy after both. AI fallback NOT live-wired into ingest — explicitly deferred to a next separate PR.
...
- **Implementation carrier**: PR #627 (branch `fix/cif-authority-resolver-tristate`, commits `7a15d74` + `87c4548`). Not yet merged/deployed as of 2026-06-16.

## Next 3 actions in queue

1. **Operator review + merge PR #632** (OCR/AI image-only extraction fallback) — target: operator merge decision; gating: GATE 1 already satisfied (all CRITICAL/HIGH resolved inline); deploy is a separate operator step requiring the full 7-agent gate with `deploy-security-reviewer` as canonical security reviewer (not `security-permissions` — see OQ-OCR-GATE5); backend-safety-reviewer re-run on vision write paths required before deploy gate fires (see OQ-OCR-BACKEND-SAFETY).
2. **Deploy CIF authority bundle** (#625 + #627 + #631 now on main, NOT YET in C:\PZ) — target: operator-gated 7-agent `/deploy` for the CIF tri-state + AWB extraction hardening + e2e regression bundle; gating: financial-adjacent, requires operator approval + explicit deploy gate run; production at `92fe65b` (E3b deploy), main at `a421fe9`.
3. **Validate flow-context-keeper dispatch** in a fresh session — target: confirm `model: sonnet` frontmatter fix took effect so PROJECT_STATE updates can be agent-dispatched rather than hand-written; gating: merge of config PR + `git pull` into `C:\Users\Super Fashion\PZ APP\.claude\agents\` tree (operator-territory sync); see OQ for full resolution steps.
...
## OQ-OCR-GATE5: PR #632 deploy gate must use deploy-security-reviewer, not security-permissions (GATE 5 SCHEDULED — 2026-06-17)
...
## OQ-OCR-BACKEND-SAFETY: backend-safety-reviewer fuller sweep of vision write paths required before PR #632 deploy gate (GATE 4 SCHEDULED — 2026-06-17)
...
## ~~OQ-PR627: PR #627 CIF tri-state resolver — awaiting review/merge; deploy gated on operator approval (2026-06-16)~~ MERGED as `e4d96b5` (2026-06-16); see PRs #625/#627/#631/#628 MERGED FACTS entry

- **Resolution (2026-06-16)**: PR #627 MERGED to main as `e4d96b5`. PR #625 MERGED as `729afe2`. PR #631 MERGED as `a421fe9` (e2e regression + merge-not-downgrade fix). All three on origin/main. NOT YET DEPLOYED to C:\PZ — see "Next 3 actions" item 2 and OQ below.

## OQ-PR627-DEPLOY: CIF authority bundle (#625 + #627 + #631) merged but NOT deployed to C:\PZ (2026-06-17)

- **Question**: When will the CIF tri-state resolver + AWB extraction hardening + e2e regression bundle be deployed to production `C:\PZ`?
- **Who can answer**: Operator — deploy decision (financial-adjacent change; full 7-agent `/deploy` gate required; no autonomous deploy permitted).
- **Impact if left unanswered**: Production continues to use pre-fix `float(... or 0)` CIF coercions in clearance decisions. AWB 2315714531 (true CIF USD 732) continues to show `total_value_usd=0.0`. FedEx path silent-zero gap persists.
- **Preconditions for deploy**: origin/main at `a421fe9` (confirmed) + clean worktree from `git worktree add C:\PZ-release origin/main` + standard `service/app` robocopy + recursive pycache purge + PZService restart + hash-flip verification on `cif_resolver.py`, `routes_upload.py`, `routes_clearance.py` + clearance endpoint smoke (`GET /api/v1/shipment/<awb>/clearance` must not return `total_value_usd=0.0` for AWB 2315714531).
- **Candidate closure path**: operator approves → 7-agent gate → deploy → verify hash-flip + clearance endpoint → OQ closes to FACTS.

## ~~OQ-PR627: PR #627 CIF tri-state resolver — awaiting review/merge; deploy gated on operator approval (2026-06-16)~~ MERGED as `e4d96b5` 2026-06-16; deploy OQ moved to OQ-PR627-DEPLOY above

- ~~**Question**: When will PR #627 (`fix/cif-authority-resolver-tristate`, commits `7a15d74` + `87c4548`) be reviewed and merged? And separately, when will the 7-agent deploy gate run for production sync?~~
- **Resolution (merge)**: PR #627 MERGED as `e4d96b5` on 2026-06-16. PR #625 MERGED as `729afe2`. PR #631 MERGED as `a421fe9` (e2e regression). Deploy question transferred to OQ-PR627-DEPLOY above.
- ~~**GATE 2 note**: 3/3 impl PRs open (#626, #625, #627); operator must clear at least 1 before a 4th impl PR may open.~~ — RESOLVED: PRs #626/#625/#627/#631 all merged; GATE 2 at 2/3 (#632, #630).
