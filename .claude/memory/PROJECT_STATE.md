...
Owned by `flow-context-keeper`. Do not edit by hand outside of an emergency. Last updated on 2026-06-15 (Campaign 03 Sprint 03.2 PR #608 merged to main as `1909fcc` — Shipment Detail V2 authority-honest UX polish, MERGED-NOT-DEPLOYED; origin/main HEAD now **1909fcc**; **PRODUCTION REMAINS d37316e** — neither #608 nor #602/f8108ae deployed yet; GATE 2 now **0/3 open impl PRs**; NOTE: `flow-context-keeper` is non-dispatchable this session — it carries no explicit `model:` field and inherits the harness-default subagent model `claude-sonnet-4-20250514`, which is unavailable in this environment (a tool-level `model: sonnet` override is also ignored — Lesson B: the subagent registry is fixed at session start), so this update was hand-written under the emergency clause; canonical fix = `model: sonnet` added to the agent frontmatter, pending next-session validation; see #608 FACT + the flow-context-keeper OQ). Prior: PR #602 resolves Issue #598; docs PRs #603/#605/#607 merged; #604 closed unmerged.
