...
## PR #699 — fix(proforma-v2): draft-scoped documents + single readiness authority + draft-scoped warehouse/wfirma gate (2026-06-21, MERGED `0de180f` — NOT independently verified this session)

- **PR #699 SQUASH-MERGED** to `origin/main` as `0de180f` (merged 2026-06-21, after PR #698). Title: `fix(proforma-v2): draft-scoped documents + single readiness authority + draft-scoped warehouse/wfirma gate`. **CONTEXT NOTE**: PR #699 is recorded as a FACT (it is on origin/main per operator-provided context) but was NOT independently verified by the session that recorded PR #698. No browser smoke, no hash-flip verification, no deploy gate record exists for #699 in this session's evidence chain. A future session must independently verify scope, deploy status, and test coverage.

## PR #698 — chore(v2): proforma-list Retry uses shared Btn + data-testid (2026-06-21, MERGED `b47ca02` — DEPLOYED + LIVE-VERIFIED)

- **PR #698 SQUASH-MERGED** to `origin/main` as `b47ca02` (merged 2026-06-21). Title: `chore(v2): proforma-list Retry uses shared Btn + data-testid (frontend-design §4/§8)`. Base: `main`. Branch deleted post-merge.
- **Scope — static asset only; 2 files; no backend, no schema, no engine change**:
  - `service/app/static/v2/proforma-list.jsx` — Retry button replaced with shared `Btn` component (ghost+small: borderRadius 6px, transparent bg, color `--text-2`, 1px solid transparent border, inline-flex, 11px/600, padding 4px 10px); `data-testid="btn-proforma-list-retry"` added on the rendered `<button>` element; real `draftsHook.reload` wiring preserved unchanged. Raw `<button>` callsite removed; zero `>Retry</button>` remaining.
  - `service/tests/test_v2_proforma_list_retry_btn.py` — guard test pinning the Btn contract.
  - No other static asset, no engine/root/config file touched. V1 pages (`dashboard.html`, `shipment-detail.html`) untouched.
- **DEPLOYED to production** (2026-06-21): operator-run scoped `Copy-Item` of `service/app/static/v2/proforma-list.jsx` to `C:\PZ\app\static\v2\proforma-list.jsx`. Static asset — NO PZService restart required. Pre-deploy hash: `25F45E54…`; post-deploy hash: `C9220590DD633A7A50B88ADCFC9027E3CDE0F511FAF9564FF43752BE1E96802A` (LF-normalized SHA256, full). Verified MATCH vs merged source on `origin/main`.
- **LIVE browser verification PASS** (2026-06-21, https://pz.estrellajewels.eu, authenticated): `/v2/proforma-list.jsx` served HTTP 200; source contains `btn-proforma-list-retry` + `>Retry</Btn>`, zero `>Retry</button>`. Forced drafts-load-error state (non-destructive at `window.fetch` layer, PzApi frozen, real `draftsHook.reload` used): Retry rendered via shared `Btn` (ghost+small styling confirmed: borderRadius 6px, transparent bg, color `--text-2`, 1px solid transparent border, inline-flex, 11px/600, padding 4px 10px); `data-testid="btn-proforma-list-retry"` present on the `<button>`. Real Retry click re-fired `draftsHook.reload` — drafts fetch counter 1→2. Zero console errors (only benign pre-existing in-browser Babel-precompile WARNING; `@babel/standalone@7.26.4` pin confirmed). No production data mutated.
- **GATE 1 reviewers**: reviewer-challenge CLEAR; frontend-flow-reviewer CLEAR. GATE 1 fully satisfied before PR open.
- **Lesson J N/A**: change is in `service/app/static/v2/` — standard robocopy path; no root-engine or out-of-scope-path sync required.
...
## Next 3 actions in queue (refreshed 2026-06-21 — PR #693 MERGED `3b14825`: fail-closed fix; #692 CLOSED superseded; #695 OPEN docs)

1. **Merge PR #695 (docs/overbill-tolerance-comment)** — target: operator review + merge; comment-only + this PROJECT_STATE update + scorecard citation; zero blast radius. Gating: OQ-PR695-MERGE open; no deploy gate required.
2. **Execute agent-tuning Issue #694** — target: backend-safety-reviewer evidence-packaging prompt hardened; GATE 4 ISSUE disposition executed; closes the 3-run REPEATED-WEAK pattern. Gating: OQ-PR694-ISSUE-EXECUTE open; requires agent-prompt-refiner session or direct prompt edit.
3. **Deploy PR #673 + #675 + #677 + combined delta to production** — target: 7-agent gate pass + operator robocopy + GATE-6 live browser verify (proforma authority panel, contractor-at-birth backfill for `SHIPMENT_9158478722`). Gating: OQ-PR675-DEPLOY and OQ-PR677-DEPLOY both open; operator must initiate 7-agent gate covering combined `f652de0` + `7b94a73` + `308145d` + subsequent merged-main delta.
