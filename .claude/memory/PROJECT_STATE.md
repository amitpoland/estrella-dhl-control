...
- **ADR-030 added on #640** (`.claude/adr/ADR-030-invoice-extraction-authority-separation.md`): permanent four-layer invoice authority law — (1) `vision_invoice` proposal / (2) `operator_confirmed` adoption / (3) `engine_parsed` accounting / (4) `clearance_decision` customs. Enforcement rule: no service may read `vision_invoice` to drive PZ/wFirma/landed-cost/exports/warehouse booking unless `operator_confirmed == true`. ADR README index repaired (ADR-020..ADR-030). Companion docs on branch: `service/docs/runbook-invoice-image-only-extraction-sequence.md`, `service/docs/awb-2315714531-extraction-handoff.md`, `service/docs/pr640-deployment-readiness-state.md`.
...
## Next 3 actions in queue (refreshed 2026-06-17 — PR #633 CIF-UI resolved-authority OPENED)

1. **Operator: review + merge PR #633** (branch `fix/cif-ui-resolved-authority` @ `49f1060`; GATE 1 satisfied; 3 reviewers EXEMPLARY; zero backend route schema changes; 7 tests pass; smoke clean). Then run 7-agent deploy gate (standard service/app robocopy, no Lesson J engine files) and deploy to `C:\PZ`. Verify AWB 2315714531 no longer shows blocking CIF=0 warning; Polish Description gate unblocked. Gating: operator review of PR #633; GATE 2 has 1 slot remaining (2/3 used).
2. **Operator: merge + deploy PR #632** (OCR/AI image-only extraction fallback; 7-agent gate = READY-TO-DEPLOY, conditions A & B verified green). May sequence with or after #633 — both are standard service/app syncs, no Lesson J engine files, no schema. Gating: operator action (deploy-guard hook blocks agent writes into `C:\PZ`); reports "deployed" only after prod hash flips.
3. **File reviewer environment-disclosure gap as a tuning issue** (OQ-CIF633-ENV-DISCLOSURE — all 3 review agents scored Environment 2/5; none self-declared working-tree path/branch/SHA in verdict blocks; recommend PATH GUARD self-declaration language added to reviewer prompt templates via agent-prompt-refiner). Gating: none (operator can file immediately or queue for next agent-prompt-refiner session).
