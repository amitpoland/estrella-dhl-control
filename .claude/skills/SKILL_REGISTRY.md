...
Generated 2026-06-06 by direct inspection. 9 project skills (ej-dashboard-design added 2026-07-04; ej-dashboard-fullstack-governance added 2026-07-04; ej-dashboard-webapp-testing added 2026-07-04; ej-dashboard-clean-code added 2026-07-04; ej-dashboard-master router added 2026-07-04; wfirma-api-integration reference restored 2026-07-04).
...
| `wfirma-api-integration` | Reference / knowledge | wFirma API behavior reference (auth · request/response conventions · invoices/KSeF · contractors · warehouse-goods · expenses · webhooks · currency/WDT · series/numbering · query syntax · error handling · gotchas) + pomoc.wfirma.pl UI-behavior index | Acting as an authority or implementer; **bypassing protected-domain approval** for invoices/VAT/accounting/document-generation/webhooks/API-authority/persistence; authorizing any fiscal write (`WFIRMA_CREATE_*`) — those stay gated by `ej-dashboard-fullstack-governance` | Read-only API reference |
...
- **Purpose:** Deep read-only **reference/knowledge** for the wFirma.pl API (`api2.wfirma.pl` / doc.wfirma.pl) and its product/UI behavior (pomoc.wfirma.pl) — invoices/KSeF, contractors, warehouse-goods (note: warehouse *documents* PZ/WZ/RW/PW/MM are NOT API-writable), expenses, webhooks, auth (API Key), currency/WDT-NBP, series/numbering, query syntax, error handling, gotchas, and the Estrella proforma→reservation→invoice flow. SKILL.md + 17 `references/*.md`.
- **Invocation context:** Consult whenever building/debugging/reviewing any wFirma integration or diagnosing a wFirma API error. It **informs; it never acts.** Read the matching `references/*.md` before writing any wFirma request / query / webhook code.
- **Allowed domains:** wFirma API mechanics + UI-behavior reference only.
- **Forbidden domains:** Being treated as an authority or implementer; **bypassing protected-domain approval** for invoices/VAT/accounting/document-generation/webhooks/API-authority/persistence; authorizing fiscal writes (`WFIRMA_CREATE_*` / Post / Convert / Approve). All actual wFirma writes remain gated by `ej-dashboard-fullstack-governance` (Master→Mirror→wFirma chain + protected-domain stop-and-ask).
- **Relationship / provenance:** Authored in Claude.ai from real wFirma web research (doc.wfirma.pl / pomoc.wfirma.pl / forum.wfirma.pl), delivered as a `.skill` bundle (SKILL.md + 17 references), restored to `.claude/skills/wfirma-api-integration/` on 2026-07-04. **Subordinate to `ej-dashboard-fullstack-governance`.** Fills the pre-existing `wfirma` PLANNED routing slot (`SKILL_ROUTING.md`) — a planned installation, not new unplanned sprawl, so it is consistent with the Skill Freeze Policy (which governs the seven governance/design skills, a separate category).
- **Key rules it enforces (self-stated):** never fabricate wFirma field names / endpoints / error codes; read `references/gotchas.md` before any non-trivial call; confirm auth = API Key; recommend `test.api2.wfirma.pl` when docs are incomplete; treat warehouse documents as NOT API-writable.
- **Example:** "Before wiring a wFirma invoice-create path: read `wfirma-api-integration/references/{invoices,gotchas,auth}.md` for the payload + quirks, then implement under `ej-dashboard-fullstack-governance` — the actual `WFIRMA_CREATE_INVOICE` write stays protected-domain stop-and-ask."
