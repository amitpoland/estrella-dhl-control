...
**Full `service/tests` suite reconciliation — 4 stale-test-contract classes (out-of-pattern, reconciled 2026-07-31):**
A whole-suite measurement on `main`-equivalent HEAD produced **789 failed / 20419 passed / 80 skipped / 0 errors**.
Per-file isolation (each failing file re-run alone) split this cleanly: **70 contamination-only** (17 files that go
**100% green in isolation** — the order-dependent asyncio/mock-leakage class above; the eight `test_wfirma_pz_*`
files plus `test_cliq_service`, `test_authority_separation`, `test_cpa_phase4_boundary`, `test_dhl_followup_*`,
`test_remediation_b2_b3_advisory_inbox`, `test_c1b_product_write_path`, `test_carrier_doc_package`) and
**719 reproducible-in-isolation**. Every reproducible failure maps to a pre-existing non-regression class
(frozen-V1 / `dashboard.html` marker source-grep pins per Lesson F, Lesson-O X-API-Key-vs-session 401s, missing
UI artifacts incl. absent `atlas/*.html`, the intended silent-line-drop PRODUCT-authority guard, and the four
env-conditional carrier rows above). **All are OUTSIDE both metered file-globs → zero floor impact.** Four
signature groups whose failing line could have suggested a source bug were each traced to source and confirmed
**test-side** (production code correct):

| Class | × | Failing owner | Root cause / verdict |
|-------|---|---------------|----------------------|
| `sqlite3.OperationalError: ON CONFLICT … does not match any … constraint` | 29 | `test_pr2c3b_customer_master.py`, `test_pr2c3c_bulk_price_recovery.py` — their own `_make_draft` seed helpers `INSERT … ON CONFLICT(batch_id, client_name)` | `proforma_drafts` has no UNIQUE/PK on `(batch_id, client_name)` (only `id` PK, `proforma_id` UNIQUE). Test-seed defect; no route/service runs that statement. **Not a regression.** |
| `AttributeError: 'S' object has no attribute 'environment'` | 7 | `test_email_sender.py::_settings()` stub (line 27) | Lesson E SMTP env-guard `_assert_production_env_for_smtp()` (`app/services/email_sender.py:528`) reads `settings.environment`; the real `Settings` HAS it (`app/core/config.py:15`, `='dev'`), the test's stub does not. Stale stub. **Not a regression.** |
| `AttributeError: module 'app.api.routes_proforma' has no attribute '_c1f_mirror_good_id_with_fallback'` | 6 | `test_c1f_mirror_first_reads.py` | References a renamed/removed symbol; `routes_proforma` defines `_c1f_mirror_good_id` (no `_with_fallback`). Stale symbol reference. **Not a regression.** |
| `ValueError: too many values to unpack (expected 3)` | 13 | `test_intake_currency_and_pnd.py:162` + peers unpacking `extract_packing()` into 3 | `extract_packing()` returns a **4-tuple** (`rows, parser_name, parser_version, parser_diagnostic`); ALL five production callers (`routes_intake.py`, `routes_packing.py`, `invoice_packing_extractor.py`) unpack 4 correctly. Stale 3-tuple tests + one hardcoded macOS fixture path in `test_intake.py`. **Not a regression.** |

**GATE-4 disposition: SCHEDULED** — same repair campaign as the 6-module block (repoint stale seeds/stubs/symbols;
update the 3-tuple `extract_packing` callers). Not enumerated per-test here (the four owner files + signatures above
are the tracked scope). Zero new production regressions across the full suite.
...
|------|-------------|------------------|--------|
| 2026-07-31 | 260 | 604 | **Documentation-only — no floor change, no test change. Registered 4 stale-test-contract classes from a full `service/tests` suite reconciliation** (789 failed / 20419 passed / 80 skipped / 0 errors on `main`-equivalent HEAD). Per-file isolation split it into 70 contamination-only (17 files green-in-isolation, the documented asyncio/mock-leakage class) + 719 reproducible, all outside both metered globs → **zero floor impact, zero new production regressions**. The four signature groups that could have masked a source bug were each traced to source and proven test-side: `ON CONFLICT(batch_id, client_name)` seed helpers in `test_pr2c3b/​pr2c3c` (schema has no matching UNIQUE), `test_email_sender::_settings()` stub missing `environment` (real `Settings` has it; SMTP guard `email_sender.py:528` correct), `test_c1f_mirror_first_reads` referencing the removed `_c1f_mirror_good_id_with_fallback` (real symbol is `_c1f_mirror_good_id`), and 3-tuple unpacks of the 4-tuple `extract_packing()` (all 5 production callers unpack 4). See the "Full `service/tests` suite reconciliation" block above. GATE-4 disposition: SCHEDULED (repair with the existing 6-module stale-suite campaign). Deploy-metered reality unchanged: PZ 260/260, Carrier 646 pass / floor 604, root golden 160/160. |
