...
| `test_proforma_readiness_single_authority.py::test_ambiguous_design_no_blocks_approve` | chip `task_81ea7aea` (GATE-4 SCHEDULED) | REAL DEFECT PIN — readiness design-ambiguity gate fail-open since PR #684 (`edd51926`): `_derive_draft_readiness` reads `preview.get("ambiguous_design_codes")` but the key only exists nested at `preview["design_product_bridge"]["ambiguous_design_codes"]`, so `_reconcile_billed_ambiguity` always receives `{}` and the "maps to multiple product_codes" blocker + `ambiguous_designs` selector data can never be emitted. Test reshaped 2026-07-17 to the genuine post-#684 hazard shape (billed line carrying `design_no` with blank `product_code`); fails exactly on the dead gate. Proven: with the one-line fix (read the nested key) applied locally on `d5a453fd`, suite = 12/12 green. Partial mitigation: blank/unknown billed codes still hit the wfirma_products + over-bill blockers. NOT in the metered PZ/Carrier suites. Remove this row in the fix PR. |
| `test_proforma_readiness_single_authority.py::test_operator_selection_clears_ambiguity_blocker` | chip `task_81ea7aea` (GATE-4 SCHEDULED) | Same defect as above — `readiness["ambiguous_designs"]` is always `{}`, so the before-resolution assertion fails. Goes green with the same one-line fix (verified 12/12). |
| `test_proforma_readiness_single_authority.py::test_draft32_shape_reproduces_and_repairs` | chip `task_81ea7aea` (GATE-4 SCHEDULED) | Same defect as above — the Draft-#32 reproduction expects the AMBIG blocker class alongside PRODUCT + WDT; the AMBIG class is silently dropped by the dead gate. Goes green with the same one-line fix (verified 12/12). |
