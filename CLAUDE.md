...
**DEPLOYMENT GUARD SEMANTICS (2026-08-20).** Guards classify the **operation**, not the occurrence of a string. (1) Reading, diffing, hashing, searching and parsing protected deployment files -- including production files -- is permitted and must stay frictionless. (2) A protected filename in prose, a search pattern or a read command's argument is **not** execution. (3) Production deployment, production writes, runtime-config writes and PZService control remain operator-only. (4) Anything not provably read-only -- arbitrary interpreters, command substitution, output redirection, encoded commands -- is not read-only, and unknown or ambiguous execution **fails closed**. (5) There is no second deploy authority: `.claude/deploy/Deploy-PZ.ps1` executes, `deploy_authorization.py` authorizes, `deliver.py` only reports. (6) A false-positive guard block is a **usability defect to be fixed**, never workflow friction to be normalised. Classifier + read-only vocabulary: `.claude/hooks/pz-deploy-guard.py` (`classify_command`, `_segment_is_read_only`); corpus pins: `service/tests/test_deploy_guard_semantics.py`.
