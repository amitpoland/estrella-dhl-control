// ProformaDetail.jsx — Mechanical ES-module port of proforma-detail.jsx.
// No visual change. No behavior change. Module-system rewiring only.
// DO NOT EDIT this file to add features — edit the source (proforma-detail.jsx) instead.
...
async function _apiFetch(url, opts) {
  ...
  if (!res.ok) {
    ...
    try { const j = await res.clone().json(); msg = j.detail || j.error || res.statusText } catch (_) {}
...
function CancelDraftModal({ draft, liveDraft, onClose, onSuccess }) {
  ...
  const handleCancel = () => {
    ...
    setLoading(true);
    setApiError(null);
    PzApi.cancelDraft(draft.id, liveDraft.updated_at || '', reason.trim())
      .then(r => {
        if (r && r.ok) {
          onSuccess && onSuccess();
        } else {
...
function SendProformaModal({ draft, liveDraft, recipientEmail, onClose, onSuccess }) {
  ...
  const handleSend = () => {
    ...
    setLoading(true);
    setApiError(null);
    PzApi.sendProformaEmail(draft.id, {
...
function PriorInvoiceHistoryModal({ contractorId, contractorName, onClose }) {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setApiError(null);
    PzApi.getClientInvoiceLedger(contractorId, from, to)
      .then(r => {
        if (r && r.ok) {
          setLedger(r.data);
        } else {
...
function AwbGenerateModal({ batchId, prefill, onClose, onSuccess }) {
  ...
  React.useEffect(() => {
    PzApi.listBoxTypes && PzApi.listBoxTypes()
      .then(r => {
    ...
    PzApi.listCarrierServices && PzApi.listCarrierServices()
      .then(r => setServices(r && r.ok && Array.isArray(r.data) ? r.data : []))
      .catch(() => setServices([]));
    PzApi.getCarrierStatus && PzApi.getCarrierStatus()
  ...
  const handleSubmit = () => {
    ...
    PzApi.createCarrierShipment(batchId, {
        ...
        {hasOverride && (
          <button
            ...
            onClick={() => {
              ...
              PzApi.patchDraft(id, { buyer_override: {} }, updatedAt)
...
function ProformaDetailPage({ draft, onBack, onConvert }) {
  ...
  const draftHook = PzState.useDraft(draft && draft.id);
  ...
  React.useEffect(() => {
    ...
    _apiFetch(`/api/v1/proforma/draft/${draft.id}/disclose-post`)
  ...
  const previewHook = PzState.useProformaPreview(batchId, clientName);
  ...
  const loadReservationPreview = React.useCallback(() => {
    ...
    PzApi.getReservationPreview(batchId)
  ...
  const doCreateReservation = () => {
    ...
    PzApi.createReservation(batchId, clientName)
      .then(r => {
        const body = (r && r.data) || {};
        if (r && r.ok && body.ok) {
  ...
  const doResolveAmbiguity = (design, pc) => {
    ...
    PzApi.resolveDraftAmbiguity(id, design, pc)
      .then(r => {
  ...
  const doSaveEuVat = (vr) => {
    ...
    PzApi.saveCustomerMaster(cid, { vat_eu_number: vat })
      .then(r => {
  ...
  React.useEffect(() => {
    _apiFetch('/api/v1/settings/company-profile')
  ...
  React.useEffect(() => {
    ...
    _apiFetch(`/api/v1/packing/${encodeURIComponent(batchId)}/lines`)
  ...
  const handleApprove = () => {
    ...
    const id = liveDraft.id || (draft && draft.id);
    const updatedAt = liveDraft.updated_at || (draft && draft.updated_at) || '';
    PzApi.approveDraft(id, updatedAt)
      .then(r => {
        if (r && r.ok) {
  ...
  const handleSaveEdit = () => {
    ...
    PzApi.patchDraft(draft.id, patch, liveDraft.updated_at || '')
      .then(r => {
        setEditSaving(false);
        if (r && r.ok) {
          setEditMode(false);
  ...
  const handleApplyCustomerAddress = () => {
    ...
    const id = liveDraft.id || (draft && draft.id);
    const updatedAt = liveDraft.updated_at || (draft && draft.updated_at) || '';
    PzApi.applyCustomerAddress(id, updatedAt)
      .then(r => {
        if (r && r.ok) {
          draftHook && draftHook.reload && draftHook.reload();
        } else {
  ...
  const handleFetchChargeSuggestions = () => {
    ...
    const id = liveDraft.id || (draft && draft.id);
    PzApi.suggestServiceCharges(id)
      .then(r => {
        if (r && r.ok !== false) {
  ...
  const handleApplyCharge = (type) => {
    ...
    const id = liveDraft.id || (draft && draft.id);
    const updatedAt = liveDraft.updated_at || (draft && draft.updated_at) || '';
    PzApi.applyServiceCharges(id, [type], updatedAt)
      .then(r => {
        if (r && r.ok !== false) {
          draftHook && draftHook.reload && draftHook.reload();
  ...
  const handleBuyerEditSave = () => {
    ...
    PzApi.patchDraft(id, patch, updatedAt)
      .then(r => {
          ...
          <React.Fragment>
            ...
            <ServiceChargesPanel
              ...
              onDeleteCharge={(chargeId) => {
                const id = liveDraft.id || (draft && draft.id);
                PzApi.deleteServiceCharge(id, chargeId)
                  .then(r => r && r.ok && draftHook && draftHook.reload && draftHook.reload());
              }}
            />
          </React.Fragment>
...
function ReceiptConfirmBlock({ batchId }) {
  ...
  const load = React.useCallback(() => {
    if (!batchId) return;
    setLoading(true);
    PzApi.getReceiptStatus(batchId)
  ...
  const submit = () => {
    ...
    PzApi.confirmReceipt(batchId, lines)
      .then(r => {
...
function PostToWFirmaModal({ draft, liveDraft, onClose, onSuccess }) {
  ...
  const handlePost = () => {
    ...
    setLoading(true);
    setApiError(null);
    PzApi.postDraftToWfirma(draft.id, {
      confirm_token:       'YES_POST_LOCAL_PROFORMA_DRAFT_TO_WFIRMA',
