...
def test_backup_run_with_mock_auth():
    ...
    """
    from app.main import app
    from app.auth.dependencies import require_system_settings_admin

    app.dependency_overrides[require_system_settings_admin] = lambda: {
    ...
    try:
        ...
        app.dependency_overrides.pop(require_system_settings_admin, None)


def test_backup_status_with_mock_auth(tmp_path):
    """GET /status returns canonical Business Feature Completeness fields."""
    from app.main import app
    from app.auth.dependencies import require_system_settings_admin
    ...
    try:
        with patch("app.api.routes_admin_backup.get_backup_status") as mock_status:
            mock_status.return_value = {
                "healthy": True,
                "running": False,
                "last_started_at": "2026-08-14T01:00:00+00:00",
                "last_completed_at": "2026-08-14T01:00:05+00:00",
                "duration_ms": 5000,
                "processed": 30,
                "created": 30,
                "updated": 0,
                ...
                "last_backup_id": "2026-08-14-010000",
                "registry_count": 30,
            }
            client = TestClient(app)
            response = client.get("/api/v1/admin/backup/status")
            ...
            assert body["registry_count"] == 30
            assert body["last_backup_id"] == "2026-08-14-010000"
    finally:
        app.dependency_overrides.pop(require_system_settings_admin, None)