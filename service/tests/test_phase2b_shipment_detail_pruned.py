...
# ── Size & shape ────────────────────────────────────────────────────────
# B-018 CLOSED 2026-08-13 — Phase-2B prune TARGET formally RETIRED.
# Historical target (<1 MB / <14,100 lines; C6-T4 = 14,039) was breached by
# operator-approved V1 critical fixes (proforma authority UI, PZ lifecycle).
# Re-pruning those surfaces would violate Lesson M. The GROWTH RATCHET below
# remains the Lesson F freeze guard — further unapproved V1 growth still fails.
# Do NOT bump these numbers to absorb new growth without a fresh operator ruling.
...
def test_shipment_detail_html_size_ratchet():
    ...
    assert size < _RATCHET_BYTES, (
        ...
        f"needs an operator-approved critical-fix justification; "
        f"Phase-2B prune target RETIRED under B-018 — ratchet is the guard)"
    )
...
def test_shipment_detail_html_line_ratchet():
    ...
    assert line_count < _RATCHET_LINES, (
        ...
        f"{_RATCHET_LINES:,}; V1 is FROZEN per Lesson F — B-018 prune target retired)"
    )
