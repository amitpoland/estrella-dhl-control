...
def test_same_batch_reupload_is_dedup_safe(tmp_path):
    ...
    This was a strict xfail while #885 shipped — it pinned a PRE-EXISTING
    packing_db defect (proven identical on clean b123bd4c): the pack_sr-primary
    dedup in upsert_packing_lines filtered on packing_document_id, so a
    re-upload (NEW document id) never matched and every pack_sr row duplicated
    (2 -> 4). The focused packing_db correction (this PR — dedup key is now
    (batch_id, invoice_no, pack_sr), packing_document_id traceability only)
    makes the re-upload dedup-safe, so the strict xfail is converted here into
    a hard assertion per its own reason text."""
    ev = _run_fold_flow(tmp_path)
...
# ── 4. Re-upload dedup safety via /add-document ──────────────────────────────
# _run_fold_flow (above) exercises the BACKFILL second call site
# (/{batch_id}/packing_list). This covers the OTHER post-fix re-upload path:
# /add-document, which registers its own new packing document id. Same dedup
# contract — a NEW document id must still match the existing logical rows.

def _run_intake_then_add_document(tmp_path):
    """Drive the REAL app: POST /api/v1/shipment/intake with a packing file
    (mocked parser output), then re-upload the identical packing list to the
    SAME batch via /add-document. Each stage registers its own packing document
    id, so this exercises the dedup contract across document ids on the
    add-document call site."""
    import io
    import json as _json
