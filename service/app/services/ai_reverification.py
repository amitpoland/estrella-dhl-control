"""
ai_reverification.py — AI Reverification Layer (Atlas Campaign Phase 3, §1A).

The engine of detect → inbox → approve.

After document parse, an AI pass re-verifies the extracted data for correctness
before it is trusted anywhere downstream.

READS:
  (a) The ORIGINAL source document (via document_db / shipment_documents path)
      — re-read to catch parse / OCR / extraction errors
  (b) The relevant masters: supplier, client, product, HS, company profile
  (c) The paired track's lines (purchase ↔ sales)

EMITS:
  §7 inbox proposal types, each with AI confidence / verdict:
    - supplier_mismatch
    - client_mismatch
...
PROP_DISAMBIGUATION_417G            = "disambiguation_417g"
